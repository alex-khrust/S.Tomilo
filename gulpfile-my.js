let baseDir      = '.' // Base directory path without «/» at the end
...
function browsersync() {
	browserSync.init({
		server: {
			baseDir: baseDir + '/',
			middleware: bssi({ baseDir: baseDir + '/', ext: '.html' })
		},
...
function styles() {
	return src([baseDir + `/styles/${preprocessor}/*.*`, `!` + baseDir + `/styles/${preprocessor}/_*.*`])
...
async function buildhtml() {
	let includes = new ssi (baseDir + '/', 'dist/', baseDir + '/**/*.html')
	includes.compile()
...
function startwatch() {
	watch(baseDir + `/styles/${preprocessor}/**/*`, { usePolling: true }, styles)
